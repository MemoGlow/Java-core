...
1. **Single responsibility principle (SRP - Nguyên lý đơn chức năng).**
    > Một class chỉ nên giữ một trách nhiệm duy nhất, chỉ có thể sửa class với 1 lý do duy nhất.
    - Mô tả:
...
1. **Open-Closed principle (OCP - nguyên lý đóng mở).**
    ...
    - Ví dụ về class Order.
        ...
        ```
        - Về phương thức shippingCost, để tính được shipping cost thì phải dựa trên cách thức ship và số lượng ship.
        - Mỗi lần ta muốn thêm một cách thức mới thì ta sẽ thêm 1 cái if else tương ứng, điều này có thể làm mất sự ổn định của class.
